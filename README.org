...
This guide is intended for people who have never used SuperCollider before. If you already know SuperCollider you might want to check the [[*Živa quick reference][Živa quick reference]], [[*Živa for SuperCollider users][Živa for SuperCollider users]] instead.
...
All parameters (function names) try stick to conventional music notation veocabulary.
...
Note length, how long it sounds, is set with ~legato~. To play staccato notes use ~stacc~:
...
The most basic ones is setting rests in other parameter sequences. This is explained later, after we've talked about sequencing. Rhythm sequences can be set as a separate parameter also (more on this later, too).
...
Streams of hits can also be played with. To create 5 silent beats then 3 hits:
...
Rhtyhms can be applied to pattern sequences (arrays) when a rhythm is invoked on a list, the elements will replace the hits (1s) in the pattern keeping the rests. to see how it works
...
(
[
    ~lola.faster.deg([0,4].clave.pseq).cutoff([100,1000].clave.pseq),
].ziva;
)
#+end_src
...
*** note
When playing a synth, notes can be changed with:
...
To play a chord, use an array of notes:
...
See the post window. It displays all the parameter names that can be changed with function calls, and their current value.
...
The sample (group) can be changed while playing it with ~.sound(\samplename)~.
...
Then connect the first output of SC 'MIDIOut(0)' to your MIDI device in your MIDI user iterface (this is different for each operative system).
...
*** random sequence
~.prand(repeats)~ picks a random value from the list on each event (hit):
...
Ziva.boot; // this might have to be called -- it's of bug, will be fixed
#+end_src
...
Then connect each sound to the desired track use ~>>~:
...
Ziva.drums;
#+end_src
* Živa quick reference
** TODO
* Živa for SuperCollider users
** Patterns in Živa vs Patterns in SuperCollider

  Živa uses a custom syntax for patterns, but regular SuperCollider patterns can also be used. Not all SuperCollider patterns have their equivalent in Živa, but the ones I use the most do. I keep adding them as I need.
  ...
  My approach is to make every pattern equivalent return a ~Pchain~, so it can be chained to others. See Živa's implementation of ~SequenceableCollection~ extention to see some examples.
...
SynthDef(...).add;
#+end_src
As soon as the ~SynthDef~ is added it will be listed by the ~Ziva.synths~ command.
