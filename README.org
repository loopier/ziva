...
This guide is intended for people who have never used SuperCollider before. If you already know SuperCollider you might want to check [[*Živa for SuperCollider users][Živa for SuperCollider users]] instead.
...
Rhythms can be applied to pattern sequences (arrays) when a rhythm is invoked on a list, the elements will replace the hits (1s) in the pattern keeping the rests. To see how it works

Rhythms are applied on lists. Each element in the list will consecutively be added to the next hit in the rhythm:
...
Some rhythms are divided into two bars that can be played in reversed order. E.g.: clave:
