...
You can add different sample lists, from different folders, with consecutive calls to this function; beware that if there are two subfolders with the same name only the last one will prevail.

To list all loaded samples, evaluate this function and check the post winow (number between ~()~ are the number of audio files in that subdirectory):
...
which will list all SynthDefs loaded on the server.  New synths can be loaded anytime as you would normally do with:
#+begin_src sclang
SynthDef(...).add;
#+end_src
...
Živa uses a model based after music bands. There are musicians that play instruments, following scoresheets, and a band, or parts of a band that gathers playing musicians.
...
This musician needs to play an instrument, it can be either synth or a sample:
#+begin_src sclang
  ~lola = Psynth(\acid);
  // or
  ~lola = Psound(\delia);
...
2. ~.ziva~ is a function than will make the band start playing at once on the beginning of the next beat.
